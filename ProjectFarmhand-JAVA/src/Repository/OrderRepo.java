...
public class OrderRepo {
    ...
    public void saveNewOrder(Order newOrder){
        dbc.openConnection();
        try {
            dbc.st.executeUpdate("insert into orders values('"+newOrder.getUser().getUserId()+"','"+newOrder.getId()+"','"+newOrder.getDate()+"','"+newOrder.getPaidAmount()+"','"+newOrder.getOrderStatus()+"','"+newOrder.getPaymentStatus()+"','"+newOrder.getDeliveryStatus()+"')");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        dbc.closeConnection();
